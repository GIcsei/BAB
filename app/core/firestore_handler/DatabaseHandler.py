...
class Database:
    ...
    def _request(self, token, json_kwargs={}):
        ...
        if self.build_query.get("StringQuery"):
            request_object = self.requests.post(request_ref, headers=headers,
                                               data=json.dumps(FirestoreQueryBuilder("messages")
                                                               .build_query(self.build_query["StringQuery"])
                                                               )
                                               )
        else:
    ...
    def filtering(self, response_dict:Collection, filters):
        for key,value in filters.items():
            ...
            logger.warning("Can't found element for filter setting: %s : %s", key, value)
    ...
    def get(self, token=None, json_kwargs={}):
        ...
        if response_dict is None:
            raise ValueError(f"No answer had been received, document cannot be created on: {request_dict}!")
