...
def deserialize_response(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if result is None:
            raise ValueError(f"No answer had been received, document cannot be created on: {response}!")
...
class FirestoreService:
    def __init__(self, firebase):
        # keep reference to Firebase instance so token updates are visible to all calls
    ...
    def _build_url(self, path : str):
        ...
        # Check for lead tial, and remove it if neccessary:
    ...
    def get_document(self, path):
        ...
        response = self.requests.get(url, headers=self._build_headers())
        raise_detailed_error(response)
        return response.json()

    def set_document(self, path, data):
        url = self._build_url(path)
        response = self.requests.put(url, headers=self._build_headers(), data=json.dumps(data))
        raise_detailed_error(response)
        return response.json()

    def update_document(self, path, data):
        url = self._build_url(path)
        response = self.requests.patch(url, headers=self._build_headers(), data=json.dumps(data))
        raise_detailed_error(response)
        return response.json()

    def delete_document(self, path):
        url = self._build_url(path)
        response = self.requests.delete(url, headers=self._build_headers())
        raise_detailed_error(response)
        return response.json()

    def create_document(self, path, data):
        url = self._build_url(path)
        response = self.requests.post(url, headers=self._build_headers(), data=json.dumps(data))
        raise_detailed_error(response)
        return response.json()

    @deserialize_response
    def run_query(self, collection, query_string):
        url = self._build_url("runQuery")
        query_payload = FirestoreQueryBuilder(collection).build_query(query_string)
        response = self.requests.post(url, headers=self._build_headers(), data=json.dumps(query_payload))
        raise_detailed_error(response)
        return response.json()
