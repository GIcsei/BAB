...
def _validate_filename(filename: str) -> str:
    ...
    if not re.match(r"^[a-zA-Z0-9_\-\.]+\.(pkl|pickle)$", filename, re.IGNORECASE):
        raise HTTPException(status_code=400, detail="Invalid filename format; only .pkl and .pickle files are allowed")
    return filename
...
async def preview_file(
    ...
    try:
        preview = await data_service.preview_pickle_file_async(_base_dir(), user_id, filename, max_rows=rows)
